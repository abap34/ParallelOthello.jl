struct ParalellMinMax <: AbstractSolver
    max_depth::Int
end

function choice(solver::ParalellMinMax, board1::UInt64, board2::UInt64, legals::UInt64)::UInt64
...
function parallelminmax(depth::Int, board1::UInt64, board2::UInt64, max_depth::Int)
    if depth == max_depth
    ...
    if _legals == 0x0
        score = -parallelminmax(depth + 1, board1, board2, max_depth)
    else
        cand = LegalCand(_legals)
        if depth % 2 == 0
            tasks = map(cand) do legal
                parallelminmax(depth + 1, put(board1, board2, legal, "black")..., max_depth)
            end
            score = maximum((fetch(task)::Int for task in tasks))
        else
            tasks = map(cand) do legal
                parallelminmax(depth + 1, put(board1, board2, legal, "white")..., max_depth)
            end
